...
function startAutoRefresh() {
    ...
    autoRefreshTimer = setInterval(() => {
        if (currentAccount && document.getElementById('emailsPage') && !document.getElementById('emailsPage').classList.contains('hidden')) {
...
function createEmailTableRow(email) {
    ...
    const attachmentIcon = email.has_attachments ? '<span title="包含附件">📎</span>' : '';
    ...
    // 未读标记
    const unreadBadge = !email.is_read ? '<span class="email-unread-badge" title="未读邮件"></span>' : '';
    
    return `
        <tr class="clickable ${unreadClass}">
            <td onclick="showEmailDetail('${email.message_id}')">
                <div class="email-sender-cell">
                    <div class="email-sender-avatar">${email.sender_initial}</div>
                    <div class="email-sender-info">
                        <span class="email-sender-name" title="${email.from_email}">${senderName}</span>
                        <span class="email-sender-email" title="${email.from_email}">${senderDomain}</span>
                    </div>
                </div>
            </td>
            <td onclick="showEmailDetail('${email.message_id}')">
                <div class="email-subject-cell">
                    ${unreadBadge}
                    <span class="email-subject" title="${email.subject || '(无主题)'}">${email.subject || '(无主题)'}</span>
            ...
            <td onclick="showEmailDetail('${email.message_id}')">
                <span class="email-date" title="${new Date(email.date).toLocaleString()}">${formatEmailDate(email.date)}</span>
            </td>
            <td>
                <div class="email-actions-cell">
                    <button class="btn btn-primary btn-sm" onclick="showEmailDetail('${email.message_id}')" title="查看邮件">
                        👁️
                    </button>
                    ${verificationCodeBtn}
                </div>
...
async function showEmailDetail(messageId) {
    ...
    if (cachedEmail) {
        ...
        modalContent.innerHTML = '<div class="loading"><div class="loading-spinner"></div>正在加载邮件详情...</div>';
    }
    ...
    try {
        ...
        modalContent.innerHTML = '<div class="error">❌ 加载失败: ' + error.message + '</div>';
    }
}
...
function updateEmailPagination() {
    ...
    if (infoElement) infoElement.textContent = `/ 共 ${totalPages} 页 (总计 ${emailTotalCount} 封)`;
