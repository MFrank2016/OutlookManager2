...
export function SqlQueryPanel({ tableName }: SqlQueryPanelProps) {
  ...
  const handleCopyWithSeparator = async () => {
    if (!executeSql.data?.data || executeSql.data.data.length === 0) {
    ...
    try {
      ...
      // 将每行的字段用分隔符连接
      const content = rows.map((row: string[]) => row.join(separator)).join("\n");
      
      await navigator.clipboard.writeText(content);
      toast.success(`已复制 ${rows.length} 行数据到剪贴板`);
    } catch (error) {
      toast.error("复制失败");
                ...
                {executeSql.data.data && executeSql.data.data.length > 0 ? (
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                      <Label htmlFor="separator" className="text-sm whitespace-nowrap">
                        分隔符:
                      </Label>
                      <Input
                        id="separator"
                      ...
                      />
                      <Button
                        onClick={handleCopyWithSeparator}
                        variant="outline"
                        size="sm"
                        className="ml-auto"
                      >
