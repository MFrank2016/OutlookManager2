...
import { ChevronLeft, ChevronRight, Copy, Trash } from "lucide-react";
                  ...
                  <CardContent className="p-3">
                    <div className="grid gap-2.5">
                      <div className="flex items-start justify-between gap-2.5">
                        <div className="flex min-w-0 items-center gap-2.5">
                          <Avatar className="h-9 w-9 shrink-0">
                            <AvatarFallback className="bg-gradient-to-br from-[color:var(--brand)] to-[color:var(--accent)] text-xs font-semibold text-[color:var(--primary-foreground)]">
                              {email.sender_initial}
                            </AvatarFallback>
                          </Avatar>
                          <div className="min-w-0">
                            <div className="truncate text-sm font-semibold text-foreground">{sender.name}</div>
                            <div className="truncate text-[11px] text-[color:var(--text-faint)]">
                              {sender.email || email.from_email}
                            </div>
                          </div>
                        </div>
                        <div className="shrink-0 whitespace-nowrap rounded-full bg-[color:var(--surface-1)] px-2 py-1 text-[11px] text-[color:var(--text-faint)]">
                      ...
                      <div className="flex min-w-0 items-center gap-2.5">
                        ...
                        <div className="shrink-0 rounded-full bg-[color:var(--surface-1)] px-2 py-1 text-[11px] text-[color:var(--text-faint)]">
                          ID {email.message_id}
                        </div>
                        {email.verification_code ? (
                          <Button
                            variant="outline"
                            className="h-7 shrink-0 border-amber-300 bg-amber-50/90 px-2.5 text-xs font-semibold text-amber-700 hover:bg-amber-100"
                            onClick={(event) => {
